...
from segment_goose.train import Model as SegmentGooseModel
...
from bone_goose.bone_point import Model as BoneGooseModel
...
from segment_goose.cfgs.config import cfg as segment_goose_cfg
...
from bone_goose.cfgs.config import cfg as bone_goose_cfg

from time import gmtime, strftime

animals = ["sealion", "fox", "koala","goose"]

SegmentModels = [SegmentSealionModel, SegmentFoxModel, SegmentKoalaModel, SegmentGooseModel]
BoneModels = [BoneSealionModel, BoneFoxModel, BoneKoalaModel, BoneGooseModel]

segment_cfgs = [segment_sealion_cfg, segment_fox_cfg, segment_koala_cfg ,segment_goose_cfg]
bone_cfgs = [bone_sealion_cfg, bone_fox_cfg, bone_koala_cfg, bone_goose_cfg]
...
def segment_and_bone(predict_funcs, animal_idx, input_img, crf=True):
    ...
    predictions = segment_func([batch_input_img])[0][0]
    class_num = predictions.shape[2]
    ...
    norm_bones = bone_func([batch_input_img])[0][0]
    if bone_cfg.anchor_bones != None:
        norm_bones = norm_bones + np.asarray(bone_cfg.anchor_bones)
...
def predict(predict_funcs, animal_idx, input_img, crf=True, test=False, pad=False):
    ...
    predictions = segment_func([batch_segment_input])[0][0]
...
if __name__ == '__main__':
    ...
    if args.one_request:
        ...
        cv2.imwrite("output_images/detect_input.png", detect_input)
